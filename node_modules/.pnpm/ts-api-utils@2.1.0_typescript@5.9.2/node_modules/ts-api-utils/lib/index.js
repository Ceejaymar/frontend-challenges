  ...
  return forEachToken(
    ...
    (token) => {
      ...
      if (notJsx || canHaveTrailingTrivia(token)) {
        return ts9.forEachTrailingCommentRange(fullText, token.end, commentCallback);
      }
    },
    sourceFile
  );
  ...
  while (true) {
    ...
    outer: switch (parent.kind) {
      ...
      case ts9.SyntaxKind.CallExpression: {
        ...
        return isTransientSymbolLinksFlagSet(propertySymbol.links, ts9.CheckFlags.Readonly);
      }
      case ts9.SyntaxKind.PrefixUnaryExpression:
    ...
    if (getPropertyOfType(subType, name) === undefined) {
      const index =
        (isNumericPropertyName(name)
          ...
          : undefined) ?? typeChecker.getIndexInfoOfType(subType, ts9.IndexKind.String);
      if (index?.isReadonly) {
        if (seenProperty) {
          return true;
        }
