...
export function getPathsToTry(
  ...
  for (const entry of absolutePathMappings) {
    const starMatch =
      entry.pattern === requestedModule ? '' : matchStar(entry.pattern, requestedModule);
    if (starMatch !== undefined) {
      for (const physicalPathPattern of entry.paths) {
        const physicalPath = physicalPathPattern.replace('*', starMatch);
        pathsToTry.push({ type: 'file', path: physicalPath });
        pathsToTry.push(
          ...extensions.map((e) => ({ type: 'extension', path: physicalPath + e }) as TryPath)
        );
        pathsToTry.push({
          type: 'package',
        ...
        pathsToTry.push(
          ...extensions.map((e) => ({ type: 'index', path: indexPath + e }) as TryPath)
        );
      }
    }
  }
